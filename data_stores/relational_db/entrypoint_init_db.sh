...
for sql_file in /data_stores/relational_db/schema/extensions/*.sql; do
...
for sql_file in /data_stores/relational_db/schema/tables/*.sql; do
  echo "Running $sql_file"
  PGPASSWORD=$POSTGRES_PASSWORD psql -v ON_ERROR_STOP=1 -h $POSTGRES_HOST -U $POSTGRES_USER -d $POSTGRES_DB -p $POSTGRES_PORT -f "$sql_file"
done

echo "Creating functions..."
for sql_file in /data_stores/relational_db/schema/functions/*.sql; do
  echo "Running $sql_file"
  PGPASSWORD=$POSTGRES_PASSWORD psql -v ON_ERROR_STOP=1 -h $POSTGRES_HOST -U $POSTGRES_USER -d $POSTGRES_DB -p $POSTGRES_PORT -f "$sql_file"
done

echo "Creating triggers..."
for sql_file in /data_stores/relational_db/schema/triggers/*.sql; do
  echo "Running $sql_file"
  PGPASSWORD=$POSTGRES_PASSWORD psql -v ON_ERROR_STOP=1 -h $POSTGRES_HOST -U $POSTGRES_USER -d $POSTGRES_DB -p $POSTGRES_PORT -f "$sql_file"
done
...
for ref_file in /data_stores/relational_db/seed/reference/*.sql; do
...
for py_file in /data_stores/relational_db/seed/mock/*.py; do
