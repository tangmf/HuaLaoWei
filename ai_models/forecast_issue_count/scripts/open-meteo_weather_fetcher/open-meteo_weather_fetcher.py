...
# Process single row
# --------------------------
def process_row(row, config: dict, keep_map: dict) -> tuple[List[List], bool, bool]:
    id_vals = [row[k] for k in keep_map.keys()]
...
def process_csv_bulk(config: dict):
    ...
    with ThreadPoolExecutor(max_workers=config["max_workers"]) as executor:
        futures = [executor.submit(process_row, row, config, keep_map) for _, row in df_input.iterrows()]
            ...
            for future in as_completed(futures):
                ...
                if result:
                    if first_success is None:
                        ...
                        weather_cols = config["weather_api"]["query"].get("hourly", [])
                        air_cols = config["air_api"]["query"].get("hourly", [])

                        header = list(keep_map.values()) + ["weather_datetime"]
                        header += weather_cols + air_cols
