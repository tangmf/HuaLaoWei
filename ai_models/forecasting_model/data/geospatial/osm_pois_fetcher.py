...
def compute_density(issues_gdf, pois_gdf, radius_m):
    ...
    for geom in buffer_geoms:
        matches = list(sidx.query(geom, predicate="within"))
...
def enrich_issues_with_pois(cfg: dict):
    ...
    for category, tags in tags_dict.items():
        ...
        gdf_issues[f"dist_to_nearest_{category}"] = proximity
        ...
        all_features.append(f"dist_to_nearest_{category}")

    final = gdf_issues[keep_cols + all_features]
