...
if __name__ == "__main__":
    ...
    keep_map = config.get("keep_cols", {"id": "id"})
    ...
    enriched = enriched.rename(columns=keep_map)
    datetime_col = keep_map.get(datetime_col, datetime_col)
    output_cols = list(keep_map.values()) + [datetime_col, "median_income", "total_population", "average_age"]
    enriched = enriched[output_cols]

    # Replace negative socioeconomic values with NaN
    socio_cols = ["median_income", "total_population", "average_age"]
    for col in socio_cols:
        if col in enriched.columns:
            enriched.loc[enriched[col] < 0, col] = pd.NA
