...
# --------------------------
def map_points_to_boundaries(input_csv: str, keep_map: dict, 
                              boundaries: gpd.GeoDataFrame, output_csv: str):
    df = pd.read_csv(input_csv, low_memory=False)
    df = df.rename(columns=keep_map)
    keep_cols = list(keep_map.values())
...
if __name__ == "__main__":
    ...
    keep_map = config.get("keep_cols", {"id": "id"})  # input → output
    keep_cols = list(keep_map.keys())
    ...
    if input_csv and output_csv:
        map_points_to_boundaries(input_csv, keep_map, gdf, output_csv)
