...
def cluster_latlon_points(cfg: dict):
    ...
    keep_cols_map = cfg.get("keep_cols", {})
    output_csv = cfg["output_csv"]
    cluster_summary_csv = cfg.get("output_centroids_csv")

    if not isinstance(keep_cols_map, dict):
        raise ValueError("keep_cols must be a dictionary of {original_column: new_column}")

    base_cols = list(dict.fromkeys(list(keep_cols_map.keys()) + [lat_col, lon_col]))

    for col in base_cols:
        if col not in df.columns:
            raise ValueError(f"Missing required column: {col}")

    result = df[base_cols].copy().rename(columns=keep_cols_map)
