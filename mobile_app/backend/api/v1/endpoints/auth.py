...
@router.post("/signup")
async def signup(user: dict, request: Request):
    new_user = await crud_auth.create_user(user)
...
@router.post("/signin")
async def signin(user: dict, request: Request):
    user_data = await crud_auth.authenticate_user(user)
    if user_data:
