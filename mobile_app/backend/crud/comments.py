...
async def like_comment(user_id: int, comment_id: int):
...
async def create_comment(user_id: int, post_id: int, content: str, parent_comment_id: int = None):
    async with pool.connection() as conn:
        async with conn.cursor(row_factory=dict_row) as cur:
            await cur.execute(
                """
...
async def count_comment_likes(comment_id: int):
    async with pool.connection() as conn:
        async with conn.cursor(row_factory=dict_row) as cur:
            await cur.execute(
